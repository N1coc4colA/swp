...
namespace PauseMenuNS
{
    ...
    class HQuitListener : public ButtonListener
    {
    public:
        HQuitListener(HubScene &levelScene) : m_Scene(levelScene) {}
        virtual void OnPress()
        {
            m_Scene.Quit();
        }
    ...
    class HContinueListener : public ButtonListener
    {
    public:
        HContinueListener(HubScene &levelScene) : m_Scene(levelScene) {}
        virtual void OnPress()
        {
...
PauseMenu::PauseMenu(HubScene &scene) :
    UIObject(*((Scene *)(&scene))) //Little trick to allow fast conversion.
{
    m_name = "PauseMenu";
    ...
    const std::string texts[2] = { u8"Continuer", u8"Quitter" };
    ButtonListener *listener[2] = { 0 };
    listener[0] = new PauseMenuNS::HContinueListener(scene);
    listener[1] = new PauseMenuNS::HQuitListener(scene);
